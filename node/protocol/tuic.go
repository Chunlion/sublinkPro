...
func buildTuicSurgeLine(link string, config OutputConfig) (string, string, error) {
	...
	if tuic.Version == 5 {
		line = fmt.Sprintf("%s = tuic-v5, %s, %d, uuid=%s, password=%s, udp-relay=%t, skip-cert-verify=%t ,version=%s", tuic.Name, server, utils.GetPortInt(tuic.Port), tuic.Uuid, tuic.Password, true, skipCert, strconv.Itoa(tuic.Version))
	}
